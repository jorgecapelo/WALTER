## ============================================================
##  WALTER – An R package to plot worldwide climate diagrams
##  and issue multiple bioclimate and biome classifications
##  By Jorge Capelo jorge.capelo@gmail.com
##  Lisbon, Portugal
##  AI disclaimer: LLM used as info extraction & coding aid, not modelling, rule setting and validation.
##  
...
# ══════════════════════════════════════════════════════════════
#  BOX (1981 / 1996) PHENO-PHYSIOGNOMIC PLANT FUNCTIONAL TYPES
#  Reference: Box, E.O. (1981) Macroclimate and Plant Forms: An
#    Introduction to Predictive Modeling in Phytogeography. Tasks
#    for Vegetation Science 1. Junk, The Hague.
#    Box, E.O. (1996) Plant functional types and climate at the
#    global scale. Journal of Vegetation Science 7: 309-320.
#    Box, E.O. (1997) Bioclimatic position of evergreen broad-
#    leaved forests. In: Island and High-Mountain Vegetation.
#    Proc. IAVS meeting, Univ. La Laguna, Tenerife, pp. 17-38.
#
#  ****************************************************************
#  WARNING : STILL QUITE EXPERIMENTAL
#  ------------------------------------------------------------------
#  Box's original global model (TVSI, 1981/1987) classifies plant
#  types from "climatic envelopes" — empirically fitted bounds on
#  a handful of climate variables (absolute minimum temperature,
#  growing-season warmth, a hydrological/moisture budget term)
#  outside of which a pheno-physiognomic type cannot persist,
#  reduced in Box (1996) to a minimal global set of ~15 types.
#
# ══════════════════════════════════════════════════════════════

box_pft_class <- function(T, P, Tmin, Tmax, lat) {
  # T, P, Tmin, Tmax : 12-element monthly vectors (deg C, deg C, deg C, mm)
  # lat              : decimal latitude (+N / -S)
  ...
  # Absolute minimum temperature proxy — same convention already used
  # for BIO7/BIO6-type variables elsewhere in this script (min of the
  ...
  # Walter dry months (P < 2T, T > 0) — reused as a moisture-budget proxy
  # for Box's "hydrological budget" predictor (PLACEHOLDER: Box's own
  # moisture index is not confirmed from the primary source)
  dry     <- (T > 0) & (P < 2 * T)
  n_dry   <- sum(dry)
  dry_sum <- sum(dry[sum_m])

  # Aridity threshold (Koppen-B method, reused as in the Walter/Koppen
  # block above) — PLACEHOLDER stand-in for Box's moisture envelope
  f_s <- Ps / Pann
  if      (f_s >= 0.70)                Pth <- 2 * Tann + 28
  else if ((Pann - Ps) / Pann >= 0.70) Pth <- 2 * Tann
  else                                  Pth <- 2 * Tann + 14
  ...
  evbl_excluded <- box_absmin < -15   # the one literature-anchored rule

  # ── Polar / cold-limited types ────────────────────────────  # PLACEHOLDER order/cutoffs
  if (Thot < 0)
    return(list(code = "PD",  name = "Polar desert / ice"))
  if (Thot < 10)
    return(list(code = "TUN", name = "Tundra"))
  if (Tcold < -10 && !is_arid)
    return(list(code = "BFT", name = "Boreal / taiga (evergreen needleleaf)"))

  # ── Dry types ──────────────────────────────────────────────  # PLACEHOLDER cutoffs
  if (is_desert)
    return(list(code = "DES", name = "Desert / semi-desert"))
  if (is_arid)
    return(list(code = "STP", name = "Steppe / dry grassland"))

  # ── Warm, always-hot types (Tcold >= 18) ──────────────────  # PLACEHOLDER cutoffs
  if (Tcold >= 18) {
    if (n_dry == 0)
      return(list(code = "TRF", name = "Tropical ever-wet rainforest"))
    if (n_dry <= 4)
      return(list(code = "TSF", name = "Tropical seasonal / monsoon forest"))
    return(list(code = "TSG", name = "Savanna / dry tropical woodland"))
  }

  # ── Sclerophyllous evergreen — Mediterranean-type ─────────  # PLACEHOLDER cutoffs
  # (summer-dry, mild-winter climates where evergreen-broadleaf is NOT
  # excluded by cold but moisture regime favours sclerophylly over
  # laurophylly)
  if (dry_sum >= 2 && Pw > Ps && !evbl_excluded && Tcold >= -1)
    return(list(code = "SCL", name = "Sclerophyllous evergreen (Mediterranean-type)"))

  # ── Laurophyll / evergreen broadleaf (laurel forest) ──────  # anchored on box_absmin
  # This is the one boundary tied to the literature: evergreen-broadleaf
  # dominance is excluded once box_absmin < -15 deg C (Box 1996/1997).
  if (!evbl_excluded && Tann >= 12 && n_dry <= 1)
    return(list(code = "LAU", name = "Laurophyll / evergreen broadleaf (laurel forest)"))

  # ── Subtropical evergreen mixed (warm-temperate, humid) ───  # PLACEHOLDER cutoffs
  if (!evbl_excluded && Tann >= 14 && n_dry == 0)
    return(list(code = "SEM", name = "Subtropical evergreen mixed forest"))

  # ── Cool-temperate deciduous broadleaf ────────────────────  # PLACEHOLDER cutoffs
  # Falls back here where evergreen broadleaf is excluded by cold
  # (box_absmin < -15) but conditions are otherwise warm/moist enough
  # for closed-canopy forest.
  if (Thot >= 10 && n_dry <= 2 && Pann > 400)
    return(list(code = "DEC", name = "Cool-temperate deciduous broadleaf forest"))

  # ── Fallback ───────────────────────────────────────────────
  return(list(code = "UNC", name = "Unclassified / transitional"))
}


# ── Print Box pheno-physiognomic classification ───────────────
print_box <- function(T, P, Tmin, Tmax, lat, name = "") {
  ...
  bx <- box_pft_class(T, P, Tmin, Tmax, lat)
  box_absmin <- min(Tmin)

  cat("\n", drule, "\n", sep = "")
  cat("  BOX (1981/1996) PHENO-PHYSIOGNOMIC PLANT FUNCTIONAL TYPE\n")
  cat("  Box, E.O. (1996) J. Veg. Sci. 7: 309-320\n")
  if (nchar(trimws(name)) > 0)
    cat(sprintf("  Station: %s\n", name))
  cat(drule, "\n\n", sep = "")
  cat(rule, "\n", sep = "")
  cat(sprintf("  %-6s Absolute minimum temp proxy (deg C) %8.1f\n", "Tmin", box_absmin))
  lbl("Evergreen broadleaf excluded (Tmin < -15C)", if (box_absmin < -15) "YES" else "NO")
  cat(rule, "\n", sep = "")
  lbl("Code",              bx$code)
  lbl("Pheno-physiognomic type", bx$name)
  cat(drule, "\n\n", sep = "")
...
#' Draw a Walter-Lieth diagram and print six classifications
...
#'   6. Box (1981/1996) pheno-physiognomic PFT — 
#'
...
climate_diagram <- function(place, save = FALSE, outdir = ".") {
  ...
  ## ── Box (1981/1996) pheno-physiognomic PFT — SCAFFOLD ─────
  bx <- print_box(tavg, prec, tmin, tmax, lat,
                  name = station_label)
...
==========================================================
 ...
 + Loidi, Navarro-Sánchez & Vynokurov (2022)
 + Box (1981/1996) pheno-physiognomic PFT (Experimental)
 Cache dir  : ", WC_CACHE, "
